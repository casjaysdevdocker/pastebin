...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_DIR="/usr/share/webapps/pastebin"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
root_user_name="${PASTEBIN_USERNAME:-administrator}"           # root user name
root_user_pass="${PASTEBIN_PASSWORD:-$(__random_password 32)}" # root user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
[ -f "${ROOT_FILE_PREFIX}/${SERVICE_NAME}/cron_key" ] && cron_key="$(<"${ROOT_FILE_PREFIX}/${SERVICE_NAME}/cron_key")" || cron_key="$(__random_password 24))" # generate cron key
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  if [ ! -f "${ROOT_FILE_PREFIX}/${SERVICE_NAME}/cron_key" ]; then
    echo "$cron_key" >"${ROOT_FILE_PREFIX}/${SERVICE_NAME}/cron_key"
    chmod 600 "${ROOT_FILE_PREFIX}/${SERVICE_NAME}/cron_key"
  fi
  ...
  __initialize_replace_variables "/etc/pastebin/config.php"
  # custom commands
  __replace "REPLACE_CRON_KEY" "$cron_key" "/etc/pastebin/config.php"
  __replace "REPLACE_ADMIN_USERNAME" "$root_user_name" "/etc/pastebin/config.php"
  __replace "REPLACE_ADMIN_PASSWORD" "$root_user_pass" "/etc/pastebin/config.php"
  __file_copy "/config/pastebin/." "/etc/pastebin/" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  if [ ! -d "/config/pastebin" ]; then
    mkdir -p "/config/pastebin"
    __file_copy "/etc/pastebin/." "/config/pastebin/" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  fi
...
__post_execute() {
  ...
  (
    if [ -n "$cron_key" ]; then
      sleep 5 && CRON_NAME="pastebin_cron" __cron 5 curl -q -LSsf "$HOSTNAME/$cron_key"
    fi
  ) &>/dev/null &
