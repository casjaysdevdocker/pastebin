...
user_name="${MYSQL_USER_NAME:-pastebin}"                     # normal user name
user_pass="${MYSQL_USER_PASS_WORD:-$(__random_password 32)}" # normal user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
DATABASE_CREATE="${DATABASE_CREATE:-pastebin}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  # pastebin
  __replace "REPLACE_DBUSER" "$user_name" "/etc/pastebin/config.php"
  __replace "REPLACE_DBPASS" "$user_pass" "/etc/pastebin/config.php"
  __replace "REPLACE_DBNAME" "$DATABASE_CREATE" "/etc/pastebin/config.php"
...
MYSQL_SCRIPT
    ...
    if [ -f "$WWW_DIR/sql/create_tables.sql" ]; then
      mysql -u root phpmyadmin <"$WWW_DIR/sql/create_tables.sql"
    fi
    # Run this last as it will set password
    mysql -v -u $runas <<MYSQL_SCRIPT
